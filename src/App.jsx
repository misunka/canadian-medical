...
const Tag = ({col="blue",children}) => { const m={blue:{bg:T.blueL,c:T.blue},green:{bg:T.greenL,c:T.green},orange:{bg:T.orangeL,c:T.orange},red:{bg:T.redL,c:T.red}}; return <span style={{display:"inline-block",padding:"3px 10px",borderRadius:6,fontSize:11,fontWeight:600,background:m[col]?.bg,color:m[col]?.c}}>{children}</span>; };
const stag = c => ({display:"inline-block",padding:"3px 9px",borderRadius:6,fontSize:11,fontWeight:600,background:c==="g"?"#D1FAE5":c==="o"?"#FEF3C7":c==="r"?T.redL:T.blueL,color:c==="g"?"#065F46":c==="o"?T.orange:c==="r"?T.red:T.blue});
...
function FeatureA({onBack}) {
  ...
  if(view==="list") return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <Hdr title="AI Výsledky" sub="Klinické záznamy" backFn={onBack}/>
      <div style={{flex:1,overflowY:"auto",padding:16}}>
        ...
        <div style={{fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:T.g400,marginBottom:10}}>Laboratorní výsledky</div>
        {RECORDS.filter(r=>r.type==="lab").map(r=>(
          <div key={r.id} onClick={()=>open(r)} style={{background:T.white,borderRadius:12,border:`1.5px solid ${r.statusC==="r"?"#FECACA":T.g200}`,padding:16,marginBottom:10,cursor:"pointer"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div><div style={{fontSize:14,fontWeight:700,color:T.g900}}>{r.title}</div><div style={{fontSize:12,color:T.g400,marginTop:2}}>{r.date}</div></div><span style={stag(r.statusC)}>{r.statusL}</span></div>
            <div style={{fontSize:12,color:T.g500,marginBottom:10}}>{r.summary}</div>
            <div style={{paddingTop:10,borderTop:`1px solid ${T.g100}`,display:"flex",justifyContent:"space-between"}}><div style={{display:"flex",gap:6}}><Tag col="blue">Lab + vizualizace</Tag><Tag col="blue">AI chat</Tag></div><span style={{fontSize:12,fontWeight:700,color:T.blue}}>Otevřít →</span></div>
          </div>
        ))}
        <div style={{fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:T.g400,margin:"16px 0 10px"}}>Lékařské zprávy</div>
        {RECORDS.filter(r=>r.type==="record").map(r=>(
          <div key={r.id} onClick={()=>open(r)} style={{background:T.white,borderRadius:12,border:`1px solid ${T.g200}`,padding:16,marginBottom:10,cursor:"pointer"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div><div style={{fontSize:14,fontWeight:700,color:T.g900}}>{r.title}</div><div style={{fontSize:12,color:T.g400,marginTop:2}}>{r.date} · {r.doctor}</div></div><Tag col="blue">Zpráva</Tag></div>
            <div style={{fontSize:12,color:T.g500,marginBottom:10}}>{r.summary}</div>
            <div style={{paddingTop:10,borderTop:`1px solid ${T.g100}`,display:"flex",justifyContent:"space-between"}}><div style={{display:"flex",gap:6}}><Tag col="blue">AI interpretace</Tag><Tag col="green">Care Journey</Tag></div><span style={{fontSize:12,fontWeight:700,color:T.blue}}>Otevřít →</span></div>
          </div>
        ))}
      </div>
    </div>
          ...
          <FadeIn>
            ...
            {rec.results.map((r,i)=>(
              <div key={i} style={{background:T.white,borderRadius:10,border:`1px solid ${r.status!=="normal"?"#FECACA":T.g200}`,padding:14,marginBottom:10}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}><span style={stag(r.status==="normal"?"g":"r")}>{r.status==="normal"?"✓ Normální":"↑ Nad rozsahem"}</span><span style={{fontSize:11,color:T.g500}}>{r.note}</span></div>
              </div>
            ))}
          </FadeIn>
        </div>}
...
export default function App(){
  ...
  const features=[
    {key:"A",icon:<svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={T.red} strokeWidth="2" strokeLinecap="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>,title:"AI výsledky",sub:"AI Med-Explainer",desc:"2 laboratorní výsledky a 2 lékařské zprávy čekají na interpretaci.",badge:"NOVÉ",bc:"r",meta:"28. 4. 2025"},
                  ...
                  <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:6}}>
                        ...
                        <span style={stag(f.bc)}>{f.badge}</span>
                      </div>
